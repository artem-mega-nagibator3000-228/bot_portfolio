...
/skills - используй для добавления навыка для проекта

/projects - просмотр своих проектов
...
def link_project(message, data):
    ...
    bot.send_message(message.chat.id, "Введите текущий статус проекта :-D", reply_markup=gen_markup(statuses))
...
def set_skill(message, project_name, skills):
    ...
    if skill not in skills:
        bot.send_message(message.chat.id, 'Видимо, ты выбрал навык не из спиcка, попробуй еще раз!) Выбери навык', reply_markup=gen_markup(skills))
        bot.register_next_step_handler(message, set_skill, project_name=project_name, skills=skills)
        return
...
def delete_project(message, projects):
    ...
    if project not in projects:
        bot.send_message(message.chat.id, 'У тебя нет такого проекта, попробуй выбрать еще раз :-D', reply_markup=gen_markup(projects))
        bot.register_next_step_handler(message, delete_project, projects=projects)
        return
